...
const goodFooter = `  <!-- ── CONTACT STRIP: BLACK ── -->
      ...
      <div class="md:col-span-2">
        ...
        <div class="flex gap-4 mt-8">
          <a href="https://github.com/gregorius-sergio" target="_blank" rel="noopener"
            class="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white/60 hover:bg-[#E2FF00] hover:text-[#0A0A0A] transition-all text-xs font-bold">GH</a>
          <a href="https://www.linkedin.com/in/gregorius-sergio-37762b300/" target="_blank" rel="noopener"
            class="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white/60 hover:bg-[#E2FF00] hover:text-[#0A0A0A] transition-all text-xs font-bold">LI</a>
          <a href="https://www.instagram.com/gregorius_gesergio/" target="_blank" rel="noopener"
            class="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white/60 hover:bg-[#E2FF00] hover:text-[#0A0A0A] transition-all text-xs font-bold">IG</a>
        </div>
      </div>
      <div>
...
for (const f of files) {
  ...
  // Strip everything from first sign of a footer to end of file
  content = content.replace(/(?:\/\/\s*Liquid Glass Glow Effect|<!--.*?CONTACT\s*STRIP.*?-->|<section class="bg-ink py-16">|<section class="bg-\[#0A0A0A\] py-16 md:py-24 border-t border-white\/5 relative">)[\s\S]*$/gi, goodFooter);
  
  // Make active link
  let p = f.split('.')[0];
  let searchStr = 'href="' + p + '.html" id="link-' + p + '" class="text-sm font-medium text-white/60 hover:text-white transition-colors"';
  let replaceStr = 'href="' + p + '.html" id="link-' + p + '" class="text-sm font-black text-[#E2FF00]"';
  content = content.replace(searchStr, replaceStr);

  fs.writeFileSync(f, content, 'utf8');
}
console.log('Final Polish: Fixed leaked code and synchronized headers/footers.');
