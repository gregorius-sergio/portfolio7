...
const universalFooter = `  <!-- ── CONTACT STRIP: BLACK ── -->
  <section class="bg-[#0A0A0A] py-16 md:py-24 border-t border-white/5 relative">
    ...
    <div class="w-full max-w-7xl mx-auto px-5 md:px-12 relative z-10">
      <div class="glass-2-0 liquid-glass-glow rounded-[24px] md:rounded-[40px] p-8 md:p-16 flex flex-col md:flex-row items-center justify-between gap-6 md:gap-10 border border-white/10 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.8)] group">
        <div>
        ...
        <div class="flex-shrink-0">
          <a href="index.html#contact"
            class="inline-block bg-[#E2FF00] text-[#0A0A0A] font-black text-sm px-10 py-5 rounded-full hover:bg-white transition-all duration-300 transform hover:-translate-y-1 shadow-[0_8px_24px_rgba(226,255,0,0.4)] hover:shadow-none">Connect Today →</a>
        </div>
      </div>
    </div>
  </section>
  ...
  <footer class="bg-[#0A0A0A] border-t border-white/10 relative overflow-hidden">
    ...
    <div class="w-full max-w-7xl mx-auto px-5 md:px-12 py-16 md:py-20 grid grid-cols-1 md:grid-cols-4 gap-12 relative z-10">
      ...
      <div>
        ...
        <ul class="space-y-4">
          <li><a href="project-grembit.html" id="link-project-grembit" class="text-sm font-medium text-white/60 hover:text-white transition-colors">Grembit 10 Axiom</a></li>
          <li><a href="project-fronton.html" id="link-project-fronton" class="text-sm font-medium text-white/60 hover:text-white transition-colors">Grembit Fronton</a></li>
          <li><a href="project-mevers.html" id="link-project-mevers" class="text-sm font-medium text-white/60 hover:text-white transition-colors">Grembit Mevers 7</a></li>
          <li><a href="project-grembit-vada.html" id="link-project-grembit-vada" class="text-sm font-medium text-white/60 hover:text-white transition-colors">Grembit VADA</a></li>
          <li><a href="project-portfolio.html" id="link-project-portfolio" class="text-sm font-medium text-white/60 hover:text-white transition-colors">Portfolio Website</a></li>
        </ul>
      </div>
    </div>
...
for (const f of files) {
  ...
  // Strip everything from contact strip down to the end of file!
  content = content.replace(/<section class="bg-ink py-16">[\s\S]*<\/html>/i, universalFooter);
  
  // Wait, project-fronton.html was ALREADY fixed, so it doesn't have `<section class="bg-ink py-16">`.
  // Just in case it needs the footer update for the exact link matching:
  content = content.replace(/<!-- ── CONTACT STRIP: BLACK ── -->[\s\S]*<\/html>/i, universalFooter);

  // Active state modifier for linking matching current filename
  let p = f.split('.')[0];
  let pRegex = new RegExp(`href="${p}.html" id="link-${p}" class="text-sm font-medium text-white/60 hover:text-white transition-colors"`, 'i');
  content = content.replace(pRegex, `href="${p}.html" id="link-${p}" class="text-sm font-black text-[#E2FF00]"`);

  // Strip the stray \`// Liquid Glass Glow Effect\` from the end of the file if it somehow remained outside the `</html>`
  content = content.replace(/<\/html>[\s\S]*$/g, "</html>");
  
  // We should also remove the old `bg-cream py-12` section since those screenshots were removed in phase 2!
  content = content.replace(/<!-- ── SCREENSHOTS.*?-->[\s\S]*?<section class="bg-cream.*?py-12">[\s\S]*?(?=<!-- ── CONTACT STRIP|\z)/ig, ''); // Direct tag hit
  content = content.replace(/<section class="bg-cream[\s\S]*?(?=<!-- ── CONTACT STRIP)/ig, ''); 

  fs.writeFileSync(f, content, 'utf8');
}
console.log('Synchronized footers, removed leaked code, and cleaned up screenshot blocks.');
