...
for (const f of files) {
  ...
  // Strip Old Screenshots Section
  const screenshotMatch = content.match(/<!--.*?SCREENSHOTS.*?-->[\s\S]*?(?=<!--.*?CONTACT STRIP)/i);
  if (screenshotMatch) {
    content = content.replace(screenshotMatch[0], '');
  }

  // Strip fallback tag match
  const fallbackMatch = content.match(/<section class="bg-cream.*?py-12">[\s\S]*?(?=<!--.*?CONTACT STRIP)/i);
  if (fallbackMatch) {
    content = content.replace(fallbackMatch[0], '');
  }
  
  // Replace Old Contact Strip mapping and everything below with the new footer block
  const footerMatch = content.match(/<!--.*?CONTACT STRIP.*?\n[\s\S]*?<\/html>/i);
  if (footerMatch) {
    content = content.replace(footerMatch[0], newFooter);
  }

  // Active state modifier for linking matching current filename, with extra protection
  let p = f.split('.')[0];
  let pRegex = new RegExp(`href="${p}.html" class="text-sm font-medium text-white/60 hover:text-white transition-colors"`, 'ig');
  content = content.replace(pRegex, `href="${p}.html" class="text-sm font-black text-[#E2FF00]"`);

  fs.writeFileSync(f, content, 'utf8');
}
console.log('Fixed footers and cleared cream sections completely.');
